            ...
            <div class="col-md-8">
                ...
                <div class="card shadow border-0">
                    ...
                    <div class="descript_wrap white-bg">
                        ...
                        <div class="pt-3 text-end">
                            @if (Auth::check())
                                <a href="javascript:void(0);"
                                   ...
                                   class="btn {{ $savedJobCount > 0 ? 'btn-danger' : 'btn-secondary' }}"
                                   id="saveJobBtn">
                                    <i class="fa {{ $savedJobCount > 0 ? 'fa-heart' : 'fa-heart-o' }} me-1"></i>
                                    {{ $savedJobCount > 0 ? 'Saved' : 'Save' }}
                                </a>
                            @else
                            ...
                            @if (Auth::check())
                                <a href="javascript:void(0);" onclick="applyJob({{ $job->id }})" class="btn btn-primary">
                                    Apply
                                </a>
                            @else
                        ...
                        {{-- Video 22: Show applicants to the employer only --}}
                        @if (Auth::check() && Auth::user()->id == $job->user_id)
                        <div class="mt-4">
                            <h5 class="fw-bold mb-3">Applicants ({{ count($applications) }})</h5>
...
// Video 20: Save a Job via AJAX
function saveJob(id) {
    $.ajax({
        url: '{{ route("account.saveJobPost") }}',
        type: 'post',
        data: { id: id, _token: '{{ csrf_token() }}' },
        dataType: 'json',
        success: function(response) {
            if (response.status == true) {
                $('#saveJobBtn')
                    ...
                    .addClass('btn-danger')
                    .html('<i class="fa fa-heart me-1"></i> Saved');
            } else {
                alert(response.message);
            }
        },
